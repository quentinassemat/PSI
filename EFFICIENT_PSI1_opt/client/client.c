...
int client_init(client * client) {
    ...
    profil_hashed(&client->data, client->pairing);
...
int main(int argc, char* argv[]) {
    ...
    printf("\nData :\n");
    profil_print(stdout, &client_test.data, client_test.pairing);
    ...
    for (int i = 0; i<client_test.data.tab->size; i++) {
        if (hash_is_defined(client_test.data.tab, i)) {
            ...
            element_from_bytes(hci, client_test.data.tab->table[i].v);
        ...
        if (hash_is_defined(client_test.data.tab, i)) {
            for (int j = 0; j<index; j++) {
                if (index2 == index_intersection[j]) {
                    printf("%lu\n", client_test.data.tab->table[i].k);
                }
            }
            index2 += 1;
        }
